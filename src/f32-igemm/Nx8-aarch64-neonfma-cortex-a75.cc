...
public:
  void generate(bool prefetch, size_t mr, size_t nc_mod_nr, size_t kc, size_t ks, float min, float max);
};
...
void Generator::generate(bool prefetch, size_t mr, size_t nc_mod_nr, size_t kc, size_t ks, float min, float max)
{
  assert(mr == 4 || mr == 6);
  assert(nc_mod_nr < 8);
  assert(kc != 0);
  ...
  bind(l4);
  // ks loop
  subs(x9, x9, 8 * mr); // ks -= MR * sizeof(void*)
  b_hi(l1);
...
xnn_status xnn_generate_f32_igemm_ukernel_nx8__aarch64_neonfma_cortex_a75(xnn_code_buffer* code, size_t mr, size_t nc_mod_nr, size_t kc, size_t ks, const void* params) {
  xnn_status status;
  if (mr == 4) {
    status = xnn_generate_f32_igemm_ukernel_4x8__aarch64_neonfma_cortex_a75(
        code, nc_mod_nr, kc, ks, params);
  } else if (mr == 6) {
    status = xnn_generate_f32_igemm_ukernel_6x8__aarch64_neonfma_cortex_a75(
        code, nc_mod_nr, kc, ks, params);
  } else {
    XNN_UNREACHABLE;
  }
...
xnn_status xnn_generate_f32_igemm_ukernel_nx8__aarch64_neonfma_prfm_cortex_a75(xnn_code_buffer* code, size_t mr, size_t nc_mod_nr, size_t kc, size_t ks, const void* params) {
  using namespace xnnpack::aarch64;
  Generator g(code);
  assert(params != nullptr);
  const jit_gemm_params* gemm_params = static_cast<const jit_gemm_params*>(params);
  g.generate(true, mr, nc_mod_nr, kc, ks, gemm_params->f32_minmax.min, gemm_params->f32_minmax.max);
  g.finalize();
