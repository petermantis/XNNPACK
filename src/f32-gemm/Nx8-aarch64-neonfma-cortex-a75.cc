...
class Generator : public Assembler {
 ...
 public:
  void generate(bool prefetch, size_t mr, size_t nc_mod_nr, size_t kc, float min, float max);
};
...
void Generator::generate(bool prefetch, size_t mr, size_t nc_mod_nr, size_t kc, float min, float max)
{
  assert(mr == 4 || mr == 6);
  assert(nc_mod_nr < 8);
  assert(kc != 0);
...
xnn_status xnn_generate_f32_gemm_ukernel_nx8__aarch64_neonfma_cortex_a75(xnn_code_buffer* code, size_t mr, size_t nc_mod_nr, size_t kc, const void* params) {
  using namespace xnnpack::aarch64;
  Generator g(code);
  assert(params != nullptr);
  const jit_gemm_params* gemm_params = static_cast<const jit_gemm_params*>(params);
  g.generate(false, mr, nc_mod_nr, kc, gemm_params->f32_minmax.min, gemm_params->f32_minmax.max);
  g.finalize();
...
xnn_status xnn_generate_f32_gemm_ukernel_nx8__aarch64_neonfma_prfm_cortex_a75(xnn_code_buffer* code, size_t mr, size_t nc_mod_nr, size_t kc, const void* params) {
  using namespace xnnpack::aarch64;
  Generator g(code);
  assert(params != nullptr);
  const jit_gemm_params* gemm_params = static_cast<const jit_gemm_params*>(params);
  g.generate(true, mr, nc_mod_nr, kc, gemm_params->f32_minmax.min, gemm_params->f32_minmax.max);
  g.finalize();
  if (g.error() != xnnpack::Error::kNoError) {
    return xnn_status_invalid_state;
  }
